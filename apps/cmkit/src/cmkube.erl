...
do(#{name := Name,
     ...
     server := ApiServer,
     state := present,
     props := #{labels := Labels, spec := #{type := Type, ports := Ports}}} =
       Spec) ->
        ...
        ok ->
            ...
            of
                ok ->
                    ...
                    case kuberl_apps_v1_api:delete_namespaced_deployment(Ctx, Name, Ns, #{}, Opts)
                    of
                        {error, Reason, #{status := 404}} ->
                            {ok, Reason};
                        {error, E, _} ->
        ...
        ok ->
            ...
            of
                ok ->
                    ...
                    case kuberl_apps_v1_api:delete_namespaced_stateful_set(Ctx, Name, Ns, #{}, Opts)
                    of
                        {error, Reason, #{status := 404}} ->
                            {ok, Reason};
                        {error, E, _} ->
...
do(#{namespace := Ns,
     ...
     server := ApiServer,
     state := present,
     props := #{labels := Labels, spec := Spec} = Props} =
       Params) ->
    Yaml =
        #{apiVersion => <<"batch/v1">>,
          kind => <<"Job">>,
...
do(#{host := Host,
     ...
     data := Data}) ->
    Ctx = ctx:background(),
    Cfg = kuberl:cfg_with_bearer_token(
              kuberl:cfg_with_host(
                  cmkit:to_list(Host)),
              Token),
    kuberl_core_v1_api:delete_namespaced_config_map(Ctx, Name, Ns, #{}, #{cfg => Cfg}),
    kuberl_core_v1_api:create_namespaced_config_map(Ctx,
                                                    Ns,
                                                    #{apiVersion => <<"v1">>,
                                                      kind => <<"ConfigMap">>,
    ...
    case do(Params#{state => absent}) of
        {ok, _} ->
            ...
            case kuberl_core_v1_api:create_namespaced_secret(Ctx, Ns, Secret, Opts) of
                {error, #{code := Code, message := Message}, _} ->
...
do(Q) ->
    {error, #{status => not_supported_yet, query => Q}}.
...
await(#{retries := Retries,
        ...
        exact := Exact} =
          Params) ->
    FilterFun =
        case maps:get(state, Params, undef) of
            undef ->
                fun(_) -> true end;
            State ->
                fun(#{status := #{phase := Phase}}) -> Phase =:= State end
        end,
...
opts(#{api := Host, token := Token} = Spec) ->
    Opts = opts(Host, Token),
    case maps:get(tls_verify, Spec, true) of
        true ->
            Opts;
        false ->
            skip_ssl_verify(Opts)
    end.
